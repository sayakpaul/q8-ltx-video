...
def encode_prompt(pipe, prompt, negative_prompt, max_sequence_length=128):
    prompt_embeds, prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask = pipe.encode_prompt(prompt=prompt, negative_prompt=negative_prompt, max_sequence_length=max_sequence_length)
...
def load_q8_transformer(args):
    ...
    if os.path.isfile(f"{args.q8_transformer_path}/diffusion_pytorch_model.safetensors"):
        ...
        state_dict = safetensors.torch.load_file(hf_hub_download(args.q8_transformer_path), filename="diffusion_pytorch_model.safetensors")
...
def main(args):
    ...
    if args.q8_transformer_path:
        ...
        pipe = LTXPipeline.from_pretrained(
            "Lightricks/LTX-Video", text_encoder=None, torch_dtype=torch.bfloat16
        )
